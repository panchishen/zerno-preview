/* ===== Зерно · поведение полей форм =====
   ...
   3. Ручной ввод даты: точки расставляются сами после каждой пары цифр.
      Телефон: скобки и дефисы тоже расставляются сами, префикс +7 подставляется.
...
(function(){
  ...
  // ---------- Телефон ----------
  // Формат «+7 (987) 654-32-10». Префикс подставляем при фокусе, скобки и дефисы —
  // по мере набора; человеку остаётся набрать десять цифр номера.
  var ПРЕФИКС = '+7 (';
  var ГРУППЫ = [3, 3, 2, 2];
  var РАЗДЕЛИТЕЛИ = [') ', '-', '-'];

  // Цифры, набранные после префикса. Сам префикс не разбираем: его «7» — код страны,
  // а не первая цифра номера, иначе «+7 (9» читалось бы как номер, начинающийся с 79.
  function цифрыПосле(v){
    v = String(v || '');
    return (v.indexOf(ПРЕФИКС) === 0 ? v.slice(ПРЕФИКС.length) : v).replace(/\D/g, '');
  }
  // Номер приводим к десяти цифрам, что бы человек ни набрал и ни вставил.
  function цифрыТелефона(v){
    var d = цифрыПосле(v);
    // Код страны отбрасываем только когда цифр стало больше десяти: «89161234567»,
    // «79161234567», «+7 (916) 123-45-67» из буфера. Проверка по длине, а не по первой
    // цифре, — иначе пострадали бы обычные коды на 8: Петербург «812», Кемерово «384».
    if (d.length > 10 && (d[0] === '7' || d[0] === '8')) d = d.slice(1);
    return d.slice(0, 10);          // лишние цифры не берём
  }
  // Что не так с номером; пустая строка — всё в порядке.
  // Российский план нумерации (Россвязь): географические коды ABC — 3xx, 4xx, 8xx,
  // мобильные DEF — 9xx. Значит первая цифра десятизначного номера бывает только 3, 4, 8 или 9,
  // и «0…», «1…», «2…» — заведомо не номер, а опечатка.
  function ошибкаТелефона(v){
    var d = цифрыТелефона(v);
    if (d.length < 10) return 'Номер набран не полностью';
    if (!/^[3489]/.test(d)) return 'Проверьте код номера';
    return '';
  }
  // Канонический вид для отправки: +79161234567 (E.164). Проверка на клиенте — это
  // удобство, а не гарантия: на бою номер обязан ещё раз проверить сервер.
  function телефонE164(v){
    var d = цифрыТелефона(v);
    return d.length === 10 ? '+7' + d : '';
  }
  function собратьТелефон(цифры, удаление){
    if (!цифры.length) return удаление ? '' : ПРЕФИКС;
    var v = ПРЕФИКС, начало = 0;
    for (var i = 0; i < ГРУППЫ.length; i++){
      var часть = цифры.slice(начало, начало + ГРУППЫ[i]);
      начало += ГРУППЫ[i];
      v += часть;
      // разделитель ставим, только когда группа набрана целиком; при стирании не возвращаем
      if (i < РАЗДЕЛИТЕЛИ.length && часть.length === ГРУППЫ[i] && (цифры.length > начало || !удаление)) v += РАЗДЕЛИТЕЛИ[i];
    }
    return v;
  }
  function маскаТелефона(input){
    input.setAttribute('inputmode', 'tel');
    input.addEventListener('focus', function(){
      if (!input.value){
        input.value = ПРЕФИКС;
        try { input.setSelectionRange(ПРЕФИКС.length, ПРЕФИКС.length); } catch(e){}
      }
    });
    input.addEventListener('blur', function(){
      if (!цифрыТелефона(input.value).length){
        input.value = '';                       // остался один префикс — возвращаем подсказку в поле
        снятьОшибку(input);
        return;
      }
      // Про негодный номер говорим сразу на выходе из поля, а не по нажатию кнопки:
      // на каждый символ ругаться рано, а после отправки — уже поздно.
      var беда = ошибкаТелефона(input.value);
      if (беда) показатьОшибку(input, беда); else снятьОшибку(input);
    });
    // Мышь ставит курсор туда, куда попал клик, уже после подстановки префикса —
    // внутри «+7 (» ему делать нечего, возвращаем за него. Выделение не трогаем:
    // «выделить всё и набрать заново» должно работать.
    input.addEventListener('click', function(){
      if (input.selectionStart !== input.selectionEnd) return;
      if (input.selectionStart < ПРЕФИКС.length){
        try { input.setSelectionRange(ПРЕФИКС.length, ПРЕФИКС.length); } catch(e){}
      }
    });
    // Автозаполнение браузера тоже приходит событием input, поэтому нормализуем здесь же:
    // подставленное «8 916 123-45-67» станет «+7 (916) 123-45-67» без участия человека.
    input.addEventListener('input', function(e){
      var удаление = !!(e.inputType && e.inputType.indexOf('delete') === 0);
      var было = input.value;
      var курсор = input.selectionStart || 0;
      var цифрДоКурсора = курсор <= ПРЕФИКС.length ? 0 : цифрыПосле(было.slice(0, курсор)).length;
      var цифры = цифрыТелефона(было);
      // Код региона не начинается с 0, 1 и 2 — такую цифру просто не принимаем.
      // Только при наборе с клавиатуры: вставленный из буфера номер не трогаем,
      // иначе выбросили бы первую цифру и молча собрали чужой номер — про такой
      // расскажет проверка. Набранная цифра стоит первой, отбрасываем ровно её.
      if (e.inputType === 'insertText' && /^[012]/.test(цифры)) цифры = цифры.slice(1);
      var стало = собратьТелефон(цифры, удаление);
      if (стало !== было) input.value = стало;
      var поз = Math.min(ПРЕФИКС.length, стало.length), цифр = 0;   // курсор живёт после префикса
      while (поз < стало.length && цифр < цифрДоКурсора){ if (/\d/.test(стало[поз])) цифр++; поз++; }
      if (!удаление) while (поз < стало.length && !/\d/.test(стало[поз])) поз++;
      try { input.setSelectionRange(поз, поз); } catch(err){}
      input.dataset.e164 = телефонE164(стало);   // готовое значение для будущей интеграции
    });
  }
  ...
  function показатьОшибку(el, текст){
    ...
    // Исходный текст запоминаем ровно один раз. Проверка на undefined, а не на пустоту:
    // у большинства полей подсказки нет, и через || пустая строка каждый раз считалась
    // «ещё не сохранено» — со второй отправки в исходные попадал текст ошибки, и снять её
    // потом было нечем: подсказка навсегда оставалась красной строкой под полем.
    if (hint){
      if (hint.dataset.default === undefined) hint.dataset.default = hint.textContent;
      hint.textContent = текст;
    }
  ...
  window.ZernoForms = {
    ...
    цифрыТелефона: цифрыТелефона,
    ошибкаТелефона: ошибкаТелефона,
    телефонE164: телефонE164,
    фокус: фокус,
  ...
  function init(){
    ...
    document.querySelectorAll('[data-phone]').forEach(маскаТелефона);
