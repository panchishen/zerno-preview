/* ===== Зерно · модальные окна бронирования =====
   Макеты состояний: 597:2169 (универсальная форма из шапки), 793:2754 (три формы под
   конкретный сценарий — экскурсия, стол, мастер-класс). Поля и календарь — общие,
   из forms.css/forms.js.

   Устройство: инициализировать() настраивает одно окно целиком (открытие, поля,
   валидацию, отправку) и вызывается один раз на каждое окно — так у каждой формы
   свой набор полей (у универсальной есть селект «Что бронируете?», у остальных
   вместо него телефон) и своя кнопка-триггер, а код при этом не дублируется.

   Что делает каждое окно:
   1. Открывается по своей кнопке-триггеру (делегирование через document: кнопки
      могут появиться в разметке позже — например шапку вставляет chrome.js).
      Закрывается по крестику, Esc и клику мимо.
   ...
   5. Проверяет телефон, если он есть в форме: номер набран целиком и код существует
      (правила в forms.js).
...
(function(){
  ...
  function инициализировать(modal, триггеры){
    if (!modal) return;
    ...
    function закрыть(){
      ...
      // visible обязателен: hide() у ни разу не открывавшегося календаря падает
      // внутри air-datepicker (его части создаются лениво при первом показе)
      if (date && date._dp && date._dp.visible) date._dp.hide();
      if (вернутьФокус && вернутьФокус.focus) вернутьФокус.focus();
    }

    // кнопка-триггер может появиться в разметке позже — слушаем через документ
    document.addEventListener('click', function(e){
      var кнопка = e.target.closest && e.target.closest(триггеры);
      if (!кнопка) return;
      e.preventDefault();
      открыть();
    });
    ...
    function проверить(){
      ...
      // телефон: по нему подтверждают заявку, половина номера не годится
  ...
  // универсальная форма из шапки — открывает и сама кнопка (класс .hd__btn),
  // и любой элемент с data-booking-open (например пункт мобильного меню)
  инициализировать(document.getElementById('booking'), '.hd__btn, [data-booking-open]');
  // Три формы под конкретный сценарий (макет 793:2754) — свои кнопки в блоках.
  // Форма мастер-класса свёрстана и подключена, но кнопки с [data-class-open] сейчас
  // на странице нет: в блоке конференц-зала вместо формы временно показывается
  // подсказка «Страница в разработке». Вернуть форму — дописать атрибут кнопке.
