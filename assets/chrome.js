...
(function(){
  ...
  function stubs(items, сПодменю){
    return items.map(function(t){
      ...
      if (!под) return '<a href="#" data-soon' + ПОДСКАЗКА + '>' + t + '</a>';
      // у самих «Событий» подсказки нет: пункт не ведёт на страницу, а раскрывает список
      return '<span class="nav-sub">' +
        '<a href="#" data-soon aria-haspopup="true" aria-expanded="false">' + t + '</a>' +
        '<span class="nav-sub__list">' +
          под.map(function(s){ return '<a href="#" data-soon' + ПОДСКАЗКА + '>' + s + '</a>' }).join('') +
        '</span>' +
      '</span>';
    }).join('');
  }
