...
(function(){
  ...
  var LOGO_CLIENT = new URL('logo_client.png', BASE).href; // знак заказчика, растровый (макеты 744:2929, 744:3174)
  // горизонтальный знак заказчика для стики-шапки (макет 744:2929, узел 807:2568):
  // тот же колос, что в logo_hor.svg, но надпись «МУЗЕЙ ЗЕРНА» набрана гротеском, а не техно-шрифтом
  var LOGO_CLIENT_HOR = new URL('logo_client_hor.svg', BASE).href;

  // ===== Какой знак показывать =====
  // 'client' — вариант заказчика: в стики-шапке один колос, на первом экране и в футере
  //            его знак «Музей зерна» (макеты 744:2929 и 744:3174);
  // 'zerno'  — наш прежний набор (макеты 430:1292 и 171:595).
  // Обе версии живут рядом: чтобы вернуть нашу, поменяйте одно слово в строке ниже —
  // разметка, размеры и стили второго варианта никуда не делись.
  var BRAND = 'client';
  var ЗНАКИ = {
    zerno: {
      стики: { src:LOGO,        w:269, h:48  },
      герой: { src:LOGO_VERT,   w:72,  h:90  },   // на широких экранах — колос по центру шапки
      узкий: { src:LOGO_DARK,   w:269, h:48  },   // с 1024 шапка однострочная, знак горизонтальный
      футер: { src:LOGO_DARK,   w:269, h:48  }
    },
    client: {
      // макет 744:2929: в светлой стики-шапке знак с надписью, на первом экране — один колос
      стики: { src:LOGO_CLIENT_HOR, w:171, h:41 },
      герой: { src:LOGO_VERT,   w:72,  h:90  },
      узкий: { src:LOGO_CLIENT_HOR, w:171, h:41 }, // однострочная шапка: знак с надписью слева
      футер: { src:LOGO_CLIENT, w:132, h:148 }
    }
  };
  var ЗНАК = ЗНАКИ[BRAND] || ЗНАКИ.zerno;
  ...
  // «Ресторан» и «События» — не страницы, а родители выпадающих списков: в шапках
  // раскрываются по наведению (макет Dropdown 613:886), сами никуда не ведут. В футере
  // и мобильном меню родитель уходит, вместо него встают разделы плоским списком (NAV_FOOTER)
  var SUBMENU = {
    'Ресторан': ['О ресторане', 'Меню', 'Открытая кухня'],
    'События':  ['Афиша', 'Новости']
  };
  // Адреса готовых страниц (08.09.2026). Пункта нет в списке → он остаётся заглушкой
  // с подсказкой «Страница в разработке». Сейчас заглушек не осталось
  var PAGES = new URL('../', BASE);   // страницы лежат рядом с index.html, на уровень выше assets/
  var LINKS = {
    'Главная':     'index.html',
    'Музей':       'museum.html',
    'О ресторане': 'restaurant.html',
    'Меню':        'menu.html',
    'Открытая кухня': 'kitchen.html',
    'Мастер-классы': 'master-classes.html',   // бывш. conference.html — страницу переименовали
    'Афиша':       'afisha.html',
    'Новости':     'news.html',
    'Контакты':    'contacts.html'
  };
  function адрес(t){ return LINKS[t] ? new URL(LINKS[t], PAGES).href : null; }
  // Подписи, отличные от названия раздела. В футере и мобильном меню пункта-родителя
  // «Ресторан» нет (его заменяют разделы), поэтому «О ресторане» там короче — просто
  // «Ресторан» (макет футера 744:3174, узел 744:3128). Адрес берётся по настоящему
  // названию раздела, меняется только текст ссылки
  var ПОДПИСИ = { 'О ресторане': 'Ресторан' };
  // В футере и мобильном меню «События» разворачиваются в свои разделы: раскрывать по
  // наведению там нечего, а места на отдельные строки хватает — сам пункт «События» уходит.
  // Названия те же, что в выпадающем меню: «Афишу мероприятий» сократили до «Афиши» (макет 159:417)
  ...
  function brand(cls, href, title, attrs, знак){
    знак = знак || ЗНАК.стики;
    return '<div class="' + cls + '__brand">' +
      '<a class="' + cls + '__logo" href="' + href + '" title="' + title + '"' + (attrs || '') + '>' +
        '<img src="' + знак.src + '" alt="Музей зерна" width="' + знак.w + '" height="' + знак.h + '">' +
      '</a>' +
  ...
  // Пункты ведут в никуда, пока внутренних страниц нет, — по наведению (а на тач-экране
  // по тапу) объясняем это подсказкой (макет 744:3177). Рисует её tooltip.js по атрибуту.
  var ПОДСКАЗКА = ' data-tooltip="Страница в разработке"';
  // Готовая страница — обычная ссылка; текущая помечается aria-current (рисует ничего,
  // это подсказка скринридеру). Страницы ещё нет — прежняя заглушка с подсказкой
  function пункт(t, подписи){
    var текст = (подписи && подписи[t]) || t;
    var href = адрес(t);
    if (!href) return '<a href="#" data-soon' + ПОДСКАЗКА + '>' + текст + '</a>';
    var текущая = href === location.href.split('#')[0].split('?')[0];
    return '<a href="' + href + '"' + (текущая ? ' aria-current="page"' : '') + '>' + текст + '</a>';
  }
  function stubs(items, сПодменю, подписи){
    return items.map(function(t){
      var под = сПодменю && SUBMENU[t];
      if (!под) return пункт(t, подписи);
      // у родителя списка подсказки нет: пункт не ведёт на страницу, а раскрывает список
      return '<span class="nav-sub">' +
        '<a href="#" data-soon aria-haspopup="true" aria-expanded="false">' + t + '</a>' +
        '<span class="nav-sub__list">' +
          под.map(function(s){ return пункт(s, подписи); }).join('') +
        '</span>' +
      '</span>';
    }).join('');
  }
  ...
  function mobileNavHTML(){
    var пункты = stubs(NAV_FOOTER, false, ПОДПИСИ);
  ...
  function heroHeaderHTML(){
    ...
    return '<header class="hero-header">' +
      ...
      '<div class="hero-header__brand">' +
        '<img class="hero-header__logo--vert" src="' + ЗНАК.герой.src + '" alt="Музей зерна" width="' + ЗНАК.герой.w + '" height="' + ЗНАК.герой.h + '">' +
        '<img class="hero-header__logo--hor" src="' + ЗНАК.узкий.src + '" alt="Музей зерна" width="' + ЗНАК.узкий.w + '" height="' + ЗНАК.узкий.h + '">' +
  ...
  function footerHTML(isV2){
    var brandBlock = isV2
      ? brand('ft', '#', 'Зерно — наверх', ' data-top', ЗНАК.футер)
      : brand('ft', HOME, 'Зерно — на главную', '', ЗНАК.футер);
    return '<footer class="site-footer">' +
      '<div class="ft__in">' +
        brandBlock +
        // Меню футера идёт двумя колонками по 4 пункта (макет 744:3174). Обе лежат в одном
        // <nav>, чтобы у футера был один ориентир для скринридера, а display:contents
        // в chrome.css отдаёт колонки прямо во flex .ft__in — иначе space-between
        // прижал бы их друг к другу, а в макете зазоры между всеми блоками равные
        '<nav class="ft__nav" aria-label="Разделы сайта">' +
          '<div class="ft__col">' + stubs(NAV_FOOTER.slice(0, 4), false, ПОДПИСИ) + '</div>' +
          '<div class="ft__col">' + stubs(NAV_FOOTER.slice(4), false, ПОДПИСИ) + '</div>' +
        '</nav>' +
        '<div class="ft__col">' +
          phone() +
  ...
  function init(){
    // по этому атрибуту стили подбирают размеры знака — свои у каждого варианта
